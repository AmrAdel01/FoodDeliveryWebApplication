...
export default function ProductPage() {
  ...
  return <section className="page section"><div className="container"><Link className="back-link" to="/menu"><ArrowLeft size={17} />{t('common.back')}</Link><div className="details-grid"><img className="details-image" src={productImage(product.image)} alt={product.name} /><div className="details-copy"><span className="eyebrow">{product.category}</span><h1>{product.name}</h1><p>{product.description}</p><strong className="details-price">{formatMoney(product.price, i18n.language)}</strong><div className="quantity"><button onClick={() => setQuantity(Math.max(1, quantity - 1))}><Minus size={16} /></button><span>{quantity}</span><button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))}><Plus size={16} /></button></div>{notice && <div className="notice error">{notice}</div>}<button className="button primary wide" onClick={add} disabled={!product.isAvailable || product.stock < 1}>{product.stock ? t('menu.add') : t('menu.unavailable')}</button><small>{product.stock} available</small></div></div></div></section>;
}
